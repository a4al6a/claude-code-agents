...
In addition to the markdown assessment report, write a structured JSON data file for the codebase analyzer pipeline:

```json
{
  "overall_score": 0-100,
  "summary": "Brief accessibility compliance summary",
  "conformance_target": "WCAG 2.2 AA",
  "criteria_tested": 55,
  "criteria_passed": 0,
  "criteria_failed": 0,
  "criteria_not_tested": 0,
  ...
  "recommendations": [
    {
      "priority": 1,
      "title": "Fix critical accessibility barrier",
      "description": "Detailed recommendation with code example",
...
`overall_score` uses the existing formula: `((Total_Criteria - Weighted_Failures) / Total_Criteria) * 100`. This maps the accessibility conformance score directly to the 0-100 range.

The existing markdown assessment report is preserved unchanged. This JSON file is an additional output for the codebase analyzer pipeline.
