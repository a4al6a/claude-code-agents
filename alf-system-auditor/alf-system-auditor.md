...
In addition to the per-framework markdown reports, write a structured JSON data file for the codebase analyzer pipeline:

```json
{
  "overall_score": 0-100,
  "summary": "Brief compliance posture summary across all audited frameworks",
  ...
  "recommendations": [
    {
      "priority": 1,
      "title": "Remediate critical compliance gap",
...
`overall_score` = weighted average of per-framework compliance scores × 10, where each framework's 0-10 compliance score is converted to 0-100. If only one framework is audited, its score is used directly. Minimum score: 0, maximum: 100.

The existing per-framework markdown reports are preserved unchanged. This JSON file is an additional output for the codebase analyzer pipeline.
